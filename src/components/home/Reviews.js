...
//add props when connect with DB props reviews (reviews.star, reviews.comment reviews.datetime), users.username
const Reviews = () => {
    const reviews = [
        {
            star: 4,
    ...
    return (
        <Container sx={{ py: 10, width: "100%" }}>
            ...
            >
                Người dùng hài lòng với sự tiện lợi, nhanh chóng và minh bạch trong quá trình sử dụng. Họ có thể tiết kiệm thời gian, công sức và xử lý công việc hiệu quả hơn bao giờ hết. Sự chuyên nghiệp và tận tâm của đội ngũ hỗ trợ cũng góp phần tạo nên trải nghiệm tuyệt vời, khiến khách hàng hoàn toàn yên tâm khi sử dụng dịch vụ.
            </Typography>

            <Box sx={{ width: "100%", display: "flex", justifyContent: "space-between", py: 10, gap: 5, alignItems: "center", height: "200px" }}>
                <Box sx={{ width: "30%" }}>
                    ...
                    <Typography variant="body2" mt={1} color="#8791a5">
                        Dựa trên{" "}
                        <span style={{ textDecoration: "underline", color: "#8791a5" }}>
                            100.000 đánh giá
                        </span>
                    </Typography>
                </Box>
                <IconButton onClick={handlePrevReviews} sx={{
                ...
                <Box sx={{ display: "flex", justifyContent: "space-between", width: "100%", alignItems: "center" }}>
                    {currentReviews.map((review, index) => (
                        <Box key={index} sx={{ width: "33.333%", display: "flex", flexDirection: "column", alignItems: "flex-start", mr: 2 }}>
                            ...
                            <Box justifyContent="left" alignItems="flex-end" mt={1}>
                                ...
                                <Typography variant="body2" mb={0.1}>
                                    <span style={{ fontWeight: "bold", color: "#8791a5" }}>
                                        {review.username},
                                    </span>
                ...
                <IconButton onClick={handleNextReviews} sx={{
                    color: primary[500], transition: 'transform 0.3s ease',
                    '&:hover': { transform: 'scale(1.2)' }
                }}>
